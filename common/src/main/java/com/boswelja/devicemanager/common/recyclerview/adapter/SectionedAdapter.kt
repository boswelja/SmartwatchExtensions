 ...
 * contain a [String] for the section label (or [SECTION_HEADER_HIDDEN] if there shouldn't be a
 * label), and an [ArrayList] of items to show in the section.
...
abstract class SectionedAdapter<T>(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            ITEM_TYPE_SECTION_HEADER -> SectionHeaderViewHolder.from(parent)
    ...
    private fun updateItemCount() {
        itemCount = 0
        items.forEach { itemCount += it.second.count() }
    }
    ...
    private fun getItem(position: Int): T? {
        ...
        for (it in items) {
            ...
            val countIncludingSection =
                if (section != SECTION_HEADER_HIDDEN) {
    ...
    private fun sortSectionItems(sectionIndex: Int) {
        when (itemSortMode) {
            SortMode.ASCENDING -> {
                items[sectionIndex].second.sortBy { item ->
                    item.toString().toLowerCase(Locale.getDefault())
                }
            }
            SortMode.DESCENDING -> {
                items[sectionIndex].second.sortByDescending { item ->
                    item.toString().toLowerCase(Locale.getDefault())
                }
            }
            SortMode.DEFAULT -> return
     ...
     * @param sectionIndex The index of the section to remove the item from, or -1 if it's not
     * known.
     */
    fun removeItem(item: T, sectionIndex: Int = -1) {
        val realSectionIndex =
            if (sectionIndex == -1) {
    ...
    /**
     * Defines available sorting modes the adapter can use. [DEFAULT] disables sorting and sticks
     * with the default order of the items list.
     */
    enum class SortMode {
        DEFAULT,
        ASCENDING,
