...
class PhoneLockingPreferenceFragment :
  ...
  /**
   * Update [phoneLockModePreference] summary. If phone locking mode isn't an option on the device,
   ...
   * [PHONE_LOCKING_MODE_ACCESSIBILITY_SERVICE], this launches accessibility settings, otherwise it
   * launches device administrator settings.
   */
  private fun openDeviceSettings() {
    ...
    if (isInAccessibilityMode()) {
      ...
      Intent()
          .apply {
            ComponentName("com.android.settings", "com.android.settings.DeviceAdminSettings").also {
              component = it
            }
          }
