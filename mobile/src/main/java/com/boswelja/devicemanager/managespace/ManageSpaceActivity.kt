...
class ManageSpaceActivity : BaseToolbarActivity() {
  ...
  /**
   * Calls the system's [ActivityManager] clearApplicationUserData() to make sure we haven't missed
   * anything.
  ...
  private fun doFullReset() {
    ...
    coroutineScope.launch(Dispatchers.IO) {
      ...
      WidgetDatabase.open(this@ManageSpaceActivity).apply { clearAllTables() }.also { it.close() }
      withContext(Dispatchers.Main) {
        incrementProgressBar()
        setProgressStatus(
            getString(
                R.string.reset_app_resetting_database_for, getString(R.string.database_name_main)))
      }
      WatchDatabase.get(this@ManageSpaceActivity).apply { clearAllTables() }.also { it.close() }
      withContext(Dispatchers.Main) {
        incrementProgressBar()
