...
class BatterySyncPreferenceFragment :
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
        ...
        when (key) {
            ...
            BATTERY_PHONE_CHARGE_NOTI_KEY -> {
                batterySyncPhoneChargedNotiPreference.isChecked =
                    sharedPreferences?.getBoolean(key, false)!!
                setBatteryChargeThresholdEnabled()
            }
            BATTERY_WATCH_CHARGE_NOTI_KEY -> {
                batterySyncWatchChargedNotiPreference.isChecked =
                    sharedPreferences?.getBoolean(key, false)!!
                setBatteryChargeThresholdEnabled()
            }
    ...
    override fun onPreferenceChange(preference: Preference?, newValue: Any?): Boolean {
        ...
        return when (val key = preference?.key) {
            ...
            BATTERY_PHONE_CHARGE_NOTI_KEY, BATTERY_WATCH_CHARGE_NOTI_KEY -> {
                val value = newValue == true
                sharedPreferences.edit().putBoolean(key, value).apply()
                coroutineScope.launch(Dispatchers.IO) {
                    watchManager.updatePreference(connectedWatch!!, key, value)
                }
                false
            }
            else -> true
    ...
    private fun updateChargeNotiPrefSummaries() {
        ...
        batterySyncPhoneChargedNotiPreference.summary =
            getString(R.string.pref_battery_sync_phone_charged_noti_summary).format(chargeThreshold)
        batterySyncWatchChargedNotiPreference.summary =
            getString(R.string.pref_battery_sync_watch_charged_noti_summary).format(chargeThreshold)
    }
    ...
    /**
     * Sets whether battery sync is enabled or disabled. Handles everything that needs doing when
     * battery sync is toggled.
        ...
        coroutineScope.launch(Dispatchers.IO) {
            if (enabled) {
                val workerStartSuccessful =
                    BatterySyncWorker.startWorker(requireContext(), connectedWatch!!.id)
                if (workerStartSuccessful) {
                    sharedPreferences.edit(commit = true) {
                        putBoolean(BATTERY_SYNC_ENABLED_KEY, enabled).apply()
                    }
                    watchManager.updatePreference(
                        connectedWatch!!, BATTERY_SYNC_ENABLED_KEY, enabled
                    )
                    updateBatteryStats(requireContext(), connectedWatch!!.id)
                } else {
                ...
                BatterySyncWorker.stopWorker(requireContext(), connectedWatch!!.id)
            }
        }
    }
    ...
    private fun setBatterySyncInterval(newInterval: Int) {
        ...
        coroutineScope.launch(Dispatchers.IO) {
            ...
            watchManager.updatePreference(
                connectedWatch!!, BATTERY_SYNC_INTERVAL_KEY, newInterval
            )
            BatterySyncWorker.stopWorker(requireContext(), connectedWatch!!.id)
            BatterySyncWorker.startWorker(requireContext(), connectedWatch!!.id)
        }
