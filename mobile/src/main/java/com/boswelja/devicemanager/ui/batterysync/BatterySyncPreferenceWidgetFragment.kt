...
class BatterySyncPreferenceWidgetFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lastUpdateTimer.totalMinutes.observe(viewLifecycleOwner) {
            setLastUpdateTime(it)
        }
    ...
    private fun updateBatteryStats(batteryStats: WatchBatteryStats) {
        val dataAgeMinutes = TimeUnit.MILLISECONDS.toMinutes(System.currentTimeMillis() - batteryStats.lastUpdatedMillis).toInt()
    ...
    private fun setLastUpdateTime(minutes: Int) {
        val lastUpdateText = if (minutes < 1) {
