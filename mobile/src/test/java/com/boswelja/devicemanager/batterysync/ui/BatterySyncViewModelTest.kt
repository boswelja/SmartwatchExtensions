...
class BatterySyncViewModelTest {
    ...
    fun `Toggling Battery Sync preference updates the corresponding LiveData`() {
        ...
        sharedPreferences.edit(commit = true) { putBoolean(PreferenceKey.BATTERY_SYNC_ENABLED_KEY, true) }
        viewModel.batterySyncEnabled.getOrAwaitValue {
            assertThat(it).isTrue()
        }

        sharedPreferences.edit(commit = true) { putBoolean(PreferenceKey.BATTERY_SYNC_ENABLED_KEY, false) }
        viewModel.batterySyncEnabled.getOrAwaitValue {
